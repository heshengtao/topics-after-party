...
| `category` | string | 否 | Any | 话题分类过滤 (支持中英文) | `Life Planning` |
...
*   **depth (深度)**:
    ...
    *   `4`: Love (暧昧/恋爱)
    *   `5`: Soul (灵魂/哲学)
*   **category (分类)**: 可使用 `/api/categories` 接口获取最新列表。
...
### 2. 获取分类列表 (Get Category List)

获取当前所有可用的话题分类，用于前端筛选菜单展示。系统会自动根据话题数据进行去重。
...
| 参数 | 类型 | 必填 | 默认值 | 说明 |
| :--- | :--- | :--- | :--- | :--- |
| `locale` | string | 否 | `en-US` | 返回分类名称的语言。可选 `zh-CN` 或 `en-US` |
